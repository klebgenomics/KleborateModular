...
def add_cli_options(parser):
    ...
    group.add_argument('--cbst_min_identity', type=float, default=90.0,
                       help='Minimum alignment percent identity for colibactin MLST')
    group.add_argument('--cbst_min_coverage', type=float, default=80.0,
                       help='Minimum alignment percent coverage for colibactin MLST')
    group.add_argument('--cbst_required_exact_matches', type=int, default=8,
                       help='At least this many exact matches are required to call an ST')
    return group


def check_cli_options(args):
    if args.cbst_min_identity <= 50.0 or args.cbst_min_identity >= 100.0:
        sys.exit('Error: --cbst_min_identity must be between 50.0 and 100.0')
    if args.cbst_min_coverage <= 50.0 or args.cbst_min_coverage >= 100.0:
        sys.exit('Error: --cbst_min_coverage must be between 50.0 and 100.0')
    if args.cbst_required_exact_matches < 0:
...
def get_results(assembly, args, previous_results):
    ...
    st, lineage, alleles = multi_mlst(assembly, profiles, alleles, genes, 'clb_lineage',
                                      args.cbst_min_identity, args.cbst_min_coverage,
                                      args.cbst_required_exact_matches)
